 ...
 * - personal(owner) + assignee を母集団にする
 * - 完了数は「本人が完了(completedBy=本人)」のみカウント
 */
class MemberStatsQueryServiceImpl @Inject constructor(
    ...
    override fun fetchMemberStats(): List<MemberStatsQueryService.MemberStatsDto> {
        return database.withSession { dsl ->
            ...
            val ownerRows = dsl
                .select(
                    ...
                    te.STATUS.eq("COMPLETED")
                        .and(te.COMPLETED_BY_MEMBER_ID.eq(td.OWNER_MEMBER_ID))
            ...
            // assignee で対象になる行
            val assigneeRows = dsl
                .select(
                    te.ASSIGNEE_MEMBER_ID.`as`("member_id"),
                    te.ID.`as`("task_execution_id"),
                    te.STATUS.eq("COMPLETED")
                        .and(te.COMPLETED_BY_MEMBER_ID.eq(te.ASSIGNEE_MEMBER_ID))
                        .`as`("completed_by_member")
                )
                .from(te)
                .join(td).on(te.TASK_DEFINITION_ID.eq(td.ID))
                .where(baseFilter)
                .and(te.ASSIGNEE_MEMBER_ID.isNotNull)

            // personal(owner) と assignee の和集合（task_execution_id を distinct で重複排除）
            val pool = ownerRows.unionAll(assigneeRows).asTable("pool")
            ...
            val stats = dsl
                .select(
                    ...
                    DSL.coalesce(
                        DSL.countDistinct(
                            DSL.`when`(completedByMember.eq(true), taskExecutionId).otherwise(null as UUID?)
                        ),
                        0
                    ).`as`("completed_count")
                )
